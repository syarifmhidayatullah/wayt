...
func (h *QueueHandler) CallNext(c *gin.Context) {
	...
	queue, err := h.svc.CallNext(uint(id))
	if err != nil {
		response.BadRequest(c, err.Error(), nil)
		return
	}
...
func (h *QueueHandler) ListByBranch(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil {
		response.BadRequest(c, "invalid branch id", err)
		return
	}
	queues, err := h.svc.ListByBranch(uint(id))
	if err != nil {
		response.BadRequest(c, err.Error(), nil)
		return
	}
...
func (h *QueueHandler) Reset(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil {
		response.BadRequest(c, "invalid branch id", err)
		return
	}
	if err := h.svc.Reset(uint(id)); err != nil {
		response.BadRequest(c, err.Error(), nil)
		return
	}
	response.OK(c, "queue reset successfully", nil)
}
